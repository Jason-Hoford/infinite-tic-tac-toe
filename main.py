...
pygame.display.set_caption("Infinite Tic-Tac-Toe - Scrolling Tree Prototype")
...
def build_actual_tree(history):
    ...
    def generate_future(node, db, dmX, dmO, turn_p, depth):
        ...
        for vm in v_moves:
            m_str = MOVE_NAMES.get((vm[0], vm[1]), "Err")
            child = TreeNode(move=m_str, eval_score=round(random.uniform(-0.5, 0.5),1), depth_layer=node.depth_layer+1, parent=node)
...
def render_tree_recursive(node, cam_x, cam_y, zoom, clip_rect):
    ...
    depth_scale = max(0.4, 1.0 - (node.depth_layer * 0.15))
    base_radius = 24
    scaled_radius = max(6, int(base_radius * depth_scale * zoom))
    ...
    for child in node.children:
        ...
        if is_visible or clip_rect.inflate(scaled_radius*4, scaled_radius*4).collidepoint(c_screen_x, c_screen_y):
            # Draw line before node so it sits underneath
            l_thick = max(1, int(3 * depth_scale * zoom))
            pygame.draw.line(screen, TREE_LINE, (screen_x, screen_y), (c_screen_x, c_screen_y), l_thick)
            
        render_tree_recursive(child, cam_x, cam_y, zoom, clip_rect)

    if is_visible:
        render_node(node, screen_x, screen_y, scaled_radius, show_text=True, depth_scale=depth_scale, zoom_level=zoom)
